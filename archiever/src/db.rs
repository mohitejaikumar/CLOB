...
use scylla::{client::{execution_profile::ExecutionProfile, session::Session, session_builder::SessionBuilder}, frame::Compression, policies::load_balancing::{self, LoadBalancingPolicy}, statement::batch::Batch};

use crate::state::{ order::{Order}, redis_event::RedisEvent, trade::Trade, user::PostUsers, OrderId, Symbol};
...
impl ScyllaDb{
    ...
    pub fn get_order_batch_values(
        ...
        order_1.order_status = queue_trade.order_status;
        order_2.order_status = queue_trade.client_order_status;
     ...
     */
    pub async fn batch_update(&self, queue_trade: RedisEvent) -> Result<Trade, Box<dyn Error>> {
