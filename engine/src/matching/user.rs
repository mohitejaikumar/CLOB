...
impl Users {
    ...
    pub fn balance(
        ...
        let user = self.users.get(&user_id).ok_or(MatchingEngineErrors::UserNotFound)?;
    ...
    pub fn lock_amount(
        ...
        let locked_balance = user.locked_balance.get_mut(asset);

        match locked_balance {
            None => {
                user.locked_balance.insert(asset.clone(), dec!(0));
    ...
    pub fn deposit(
        ...
        let user = self.users.get_mut(&user_id).ok_or(MatchingEngineErrors::UserNotFound)?;
        let assets_balance = user.balance.get_mut(asset);
        match assets_balance {
            None => {
                user.balance.insert(asset.clone(), quantity);
            }
            Some(mut balance) => {
                balance += quantity;
            }
    ...
    pub fn validate_and_lock_limit(
        ...
        match order_side {
            OrderSide::Bid => {
                ...
                return Ok((exchange.quote.clone(), locked_amount)); 
            }
            OrderSide::Ask => {
                let locked_amount = self.validate_and_lock(
                    &exchange.base,
                ...
                return Ok((exchange.base.clone(), locked_amount));
            }
        }
    }
    ...
    pub fn validate_and_lock_market(
        ...
        match order_side {
            OrderSide::Bid => {
                let locked_balance = self.validate_and_lock(
                    &exchange.quote,
                    user_id,
                    quote
                )?;
                Ok((exchange.quote.clone(), locked_balance))
            }
            OrderSide::Ask => {
                let locked_balance = self.validate_and_lock(
                    &exchange.base,
                ...
                Ok((exchange.base.clone(), locked_balance))
            }
        }
    }
    ...
    pub fn my_assets(
        &self,
        user_id: Id
    ) -> Result<Vec<&Asset>, MatchingEngineErrors> {
        let user = self.users.get(&user_id).ok_or(MatchingEngineErrors::UserNotFound)?;
