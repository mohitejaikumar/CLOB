use crate::matching::{error::MatchingEngineErrors, limit::Limit, orderbook::Orderbook};
...
impl MatchingEngine {
    ...
    pub fn add_new_market(
        ...
        self.orderbook
            .insert(exchange.clone(), Orderbook::new(exchange));
        Ok(self)
    }
    ...
    pub fn registered_exchange(&self) -> Vec<Symbol> {
        let exchanges: Vec<Symbol> = RegisteredSymbols::iter().map(|s| s.to_string()).collect();
        exchanges
    }
    ...
    pub async fn recover_all_orderbooks(&mut self, session: &Session) {
        ...
        let res = session
            .query_unpaged("SELECT * FROM keyspace_1.user_table", &[])
            .await
            .unwrap();
