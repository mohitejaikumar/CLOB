...
use crate::matching::{Asset, Id, Quantity, Users, error::MatchingEngineErrors};
...
impl UserRequests {
    pub fn new_user(users: &mut Users, u: NewUser, con: &mut Connection) {
        let total_users = users.users.len() as u64;
        let new_user_id = total_users + 1;
        users.new_user(total_users + 1);
        let user = users.users.get(&new_user_id).unwrap();
        println!("New user created: {:?}", user);
        con.lpush::<i64, String, Value>(u.sub_id, to_string(user).unwrap())
            .unwrap();
    }
    ...
    pub fn deposit(users: &mut Users, u: Deposit, con: &mut Connection) {
        ...
        match res {
            Ok(user) => {
                println!("Deposit successful: {:?}", user);
                con.lpush::<i64, String, Value>(u.sub_id, to_string(user).unwrap())
                    .unwrap();
            }
            Err(err) => {
                println!("Deposit failed: {:?}", err);
                con.lpush::<i64, String, Value>(u.sub_id, err.to_string())
                    .unwrap();
            }
        }
    }
