use actix_web::{web::{self, Data, Json, Query}, App, HttpResponse};
use redis::Value;
use serde_json::{from_str, to_string};

use crate::{app::AppState, db::schema::{Id, User}, routes::{Deposit, GetUserBalances, NewUser, UserRequests, Withdraw}};
...
async fn get_user(
    ...
    query.sub_id = sub_id;
    let req = to_string(
        &UserRequests::GetUserBalances(query.0)
    ).unwrap();
...
pub async fn deposit(
    ...
    match response {
        Ok(_) => {
            let mut response_result: Option<String> = None;
            loop {
                let result = redis::cmd("RPOP").arg("queues:user").query::<String>(con);
                if let Ok(response) = result {
                    response_result = Some(response);
                    break;
                }
        ...
        Ok(user) => {
            ...
            match user_orders {
                ...
                Err(err) => HttpResponse::NotFound().json(
                    format!("No orders found for user: {} {}", user.id, err.to_string())
                )
            }
        }
        Err(err) => HttpResponse::InternalServerError().json(err.to_string())
