...
impl ScyllaDb {
    pub async fn new_user(&self, user: User) -> Result<(), scylla::serialize::SerializationError> {
        let s =
            r#"
        ...
        let _ = self.session.query_unpaged(s, user).await;
        Ok(())
    }
    pub async fn get_user(&self, user_id: i64) -> Result<User, scylla::serialize::SerializationError> {
        let s =
            r#"
            SELECT
                id,
        ...
        let temp = res.unwrap().into_rows_result().unwrap();
        let mut users = temp.rows::<ScyllaUser>().unwrap();
        let scylla_user = users
            .next()
            .transpose().unwrap();
        let user = scylla_user.unwrap().from_scylla_user();
        Ok(user)
    }
    pub async fn update_user(&self, user: &mut User) -> Result<(), Box<dyn Error>> {
        ...
        let s =
            ...
            WHERE id = ?;
        "#;
        let _ = self.session.query_unpaged(s, (user.balance, user.locked_balance, user.id)).await?;
        Ok(())
    }
}
