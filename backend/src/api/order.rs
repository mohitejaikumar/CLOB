use crate::db::{get_epoch_micros, schema::{Id, Order, OrderId, OrderSide, OrderStatus, OrderType, Price, Quantity, Symbol}, scylla_tables::ScyllaOrder, ScyllaDb};
...
impl ScyllaDb {
    pub async fn new_order(&self, order: Order) -> Result<(), ExecutionError> {
        let s =
            r#"
        ...
        self.session.query_unpaged(s, order).await?;
        Ok(())
    }
    ...
    pub async fn get_users_orders(&self, user_id: i64) -> Result<Vec<Order>, Box<dyn Error>> {
        ...
        let mut orders = temp.rows::<ScyllaOrder>()?;
