...
impl UserManager {
    // add the user
    // remove the user
    // assign the user_id to send order updates
    ...
    pub fn new_user (
        ...
        self.users.insert(
            ...
            UserInfo {
                ...
                ticker_subscriptions: vec![],
                depth_subscriptions: vec![]
            }
        );
        println!("New user connected");
    }

    pub fn assign_user_id(
        &mut self,
        user_addr: String,
        user_id: Option<u64>
    ){
...
impl UserManager {
    ...
    pub fn subscribe_trade(
        &mut self,
        user_addr: String,
        symbol: RegisteredSymbols
    ) {
        if let Some(user_info) = self.users.get_mut(&user_addr) {
            user_info.trade_subscriptions.push(symbol.clone());
            println!("User {} subscribed to trade updates for {}", user_addr, symbol);
        }
    }
    ...
    pub fn unsubscribe_trades(&mut self, user_addr: String, symbol: RegisteredSymbols) {
        if let Some(user) = self.users.get_mut(&user_addr) {
            ...
            println!("User {} unsubscribed to trade updates for {}", user_addr, symbol);
        }
    }
